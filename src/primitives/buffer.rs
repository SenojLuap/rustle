...
impl<const WIDTH: usize, const HEIGHT: usize> Buffer<WIDTH, HEIGHT> {
    ...
    pub fn new() -> Result<Self, &'static str> {
        ...
        Ok(Buffer{data: [[' '; WIDTH]; HEIGHT], x_clip, y_clip})
    }
    ...
    pub fn new_with_fill(fill: char) -> Result<Self, &'static str> {
        let x_clip = 0..i16::try_from(WIDTH).map_err(|_| "Window may be no wider than i32::MAX")?;
        let y_clip = 0..i16::try_from(HEIGHT).map_err(|_| "Window may be no taller than i32::MAX")?;
        Ok(Buffer{data: [[fill; WIDTH]; HEIGHT], x_clip, y_clip})
    }
    ...
    /// Draw to the specified [`DrawTarget`]
    pub fn draw_to_canvas<Pos: Into<Point>, Target: Canvas>(&self, target: &mut Target, pos: Pos) {
        let pos = pos.into();
...
impl<const WIDTH: usize, const HEIGHT: usize> Canvas for Buffer<WIDTH, HEIGHT> {
    ...
    fn draw_circle<Pos: Into<Point>>(&mut self, brush: char, pos: Pos, radius: u16) {
        ...
        Buffer::<WIDTH, HEIGHT>::draw_any_circle(radius, draw_func);
    }
