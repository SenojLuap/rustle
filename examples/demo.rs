pub fn main() {
    let mut res = Canvas::<100, 50>::new_with_fill(LIGHT_SHADE).unwrap();
    res.draw_square(PLUS_SIGN, (1, 1), 5, 5);
    res.draw_filled_square(MEDIUM_SHADE, (3, 3), 5, 5);

    res.draw_filled_circle(DARK_SHADE, (20, 20), 23);
    
    let sub_res = Canvas::<10, 2>::new_with_fill(GREEK_SMALL_LETTER_PI).unwrap();
...
use rustle::{Canvas, Blitable, Frame, FrameType, BlitTarget};
